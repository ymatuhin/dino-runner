 ...
 * @export
 */
export function Runner(outerContainerEl, params = {}) {
  ...
  this.config = params.config || Runner.config;
...
Runner.prototype = {
  ...
  isDarkMode() {
    ...
    return darkModeMediaQuery && darkModeMediaQuery.matches;
  },
  ...
  adjustDimensions() {
    ...
    const width = this.outerContainerEl.offsetWidth - padding * 2;

    this.dimensions.WIDTH = Math.min(DEFAULT_WIDTH, width);
  ...
  playIntro() {
    if (!this.activated && !this.crashed) {
      ...
      const keyframes =
        ...
        "px }" +
        "}";
      document.getElementById("offline-style").sheet.insertRule(keyframes, 0);
   ...
   */
  onKeyDown(e) {
    const focusable = ["INPUT", "BUTTON", "A", "AREA", "TEXTAREA", "SELECT", "IFRAME", "DETAILS"];
    if (focusable.includes(document.activeElement.tagName)) return;
    ...
    if (this.isCanvasInView()) {
      ...
      } else if (this.crashed && Runner.keycodes.JUMP[e.keyCode]) {
        e.preventDefault();
      }
    }
  },
   ...
   */
  adjustContainerPositionAndScale(reset) {
    // const windowHeight = window.innerHeight;
    // const scaleHeight = windowHeight / this.dimensions.HEIGHT;
    // const scaleWidth = window.innerWidth / this.dimensions.WIDTH;
    // const scale = Math.max(1, Math.min(scaleHeight, scaleWidth));
    // this.containerEl.style;
    // this.containerEl.style.transform = "translate(-50%, -50%) scale(" + scale + ")";
  },
  ...
  invert(reset) {
    if (reset) {
      this.containerEl.classList.toggle(Runner.classes.INVERTED, false);
      this.invertTimer = 0;
      this.inverted = false;
    } else {
      this.inverted = this.containerEl.classList.toggle(
